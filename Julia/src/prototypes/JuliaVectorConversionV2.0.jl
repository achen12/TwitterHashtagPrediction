...
function recurrentFixedWindow(t,dstr)
    ...
    n =  7#length(first(dstr[1])[2])
    #let's determine l, the number of windows the operation will result in.
    l = length(dstr) - t
    newdst = Array{Dict{keytype(dstr[1]),SparseMatrixCSC{UInt32,UInt64}},1}(l)
    for i in collect(1:l)
        ...
        currDict = Dict{keytype(dstr[1]),SparseMatrixCSC{UInt32,UInt64}}()
        ...
        for k in currKeys
            ...
            currDict[k]=SparseMatrixCSC{UInt32,UInt64}(sparsevec(currArr))
        end
        newdst[i] = currDict
    end
    return newdst
...
function toVectorDstr(HashTweetMap)
    ...
    ##Percent Hash Volume
    TotalVolumeMap = map(x-> sum(vcat(collect(values(x)),[0])),HashVolumeMap)
    HashPercentVolumeMap = map(i-> mapByKey(x -> round(Int,x ./ TotalVolumeMap[i].*2^16),HashVolumeMap[i]),1:length(TotalVolumeMap))
    ...
    println("Beginning 1stDeg connected Volume") 
    ...
    for i in collect(1:length(HashVolumeMap))
        ...
        for k in keys(HashVolumeMap[i])
            currDict[k] = [HashUserMap[i][k], HashPercentVolumeMap[i][k], HashVolumeMap[i][k],HashDegreeMap[i][k],HashFollowerMap[i][k],HashFriendMap[i][k],HashRTVolumeMap[i][k]]
        end
...
function toXVectors(VectorDStr,recurrentCount)
    x_vector = Array{SparseMatrixCSC{UInt32,UInt64},1}()
    HashVectorMap = recurrentFixedWindow(recurrentCount,VectorDStr)
    newdict = Array{Dict{keytype(VectorDStr[1]),UInt64},1}(length(HashVectorMap))
...
function convertToVectorFiles(dirString,dateString,windowSizeInMS = 120000,recurrentVectorSizeInWindowCount = 15,predictionCount = 30)
    ...
    trendingHashes = map(x -> Pair(round(UInt64, (x[1] - dt)/windowSizeInMS),x[2]), trendingHashes)
    sort!(trendingHashes, by = x -> x[1],rev=true)
    trendingHashes = collect(Dict(map(x -> Pair(x[2],x[1]),trendingHashes)))
    y_vector = SparseMatrixCSC{UInt8,UInt64}(spzeros(size(x_vector,2),1))
#    y_vector = Array{UInt8,1}(zeros(size(x_vector,2)))
    ...
    for x in trendingHashes
        ...
        for i in collect(0:(predictionCount-1))#Window to declare subsignal a trend from declaration.
                if (x[2]-i > 0) && (x[2]-i < length(vectorIndMapping)) && haskey(vectorIndMapping[x[2]-i],x[1])
                    if  y_vector[vectorIndMapping[x[2]-i][x[1]]] == 0 
	                y_vector[vectorIndMapping[x[2]-i][x[1]]] = predictionCount-i
                    end
                end
            end
        #end
    ...
    sort!(zh)
    println(max(length(zh),length(vectorIndMapping)));
    z = SparseMatrixCSC{UInt64,UInt32}(spzeros(length(zh),length(vectorIndMapping)))
