...
function  setupTrainingVectorBinary(x,y)
    #Find balance Sets of labels
    idySetPos = vcat(find(i-> (i== 1) || (i == 2),y))
    idySetNeg = rand(vcat(find(i-> (i != 1) && ( i != 2),y)),5*length(idySetPos))
    ...
    y = (y .==  1) + (y .== 2 )
    #Normalize X-Vector
    x = vcat(x ./ sum(x,1),log(findmax(x[1:6:96,:],1)[1])) #.- 0.5;
    #x = x ./ findmax(x,1)[1] #.- 0.5;
    return (Array{Float64,2}(x),Array{Float64,1}(y))
end
...
function MLLearn()
    ...
    (x_train, y_train) = reduce(vcatTuple, map(i -> setupTrainingVectorBinary(load(i,"x"),load(i,"y")),trainingFileSet))
    ...
    data  =  MemoryDataLayer(name="train-data",tops=[:data,:label],data=Array[x_train,y_train],batch_size=10)
    ...
    fc1   = InnerProductLayer(name="ip1",output_dim=1000,neuron=Neurons.LReLU(),bottoms=[:data], tops=[:ip1])#,weight_init=GaussianInitializer(std=10))
    #dr1   = DropoutLayer(name="dr1", ratio=0.5, bottoms=[:ip1]) #,
    fc2   = InnerProductLayer(name="ip2",output_dim=500,neuron=Neurons.LReLU(),bottoms=[:ip1], tops=[:ip2])
    fc3   = InnerProductLayer(name="ip3",output_dim=400,neuron=Neurons.LReLU(),bottoms=[:ip2],tops=[:ip3])
    fc4   = InnerProductLayer(name="ip4",output_dim=200,neuron=Neurons.LReLU(),bottoms=[:ip3],tops=[:ip4])
    fc5   = InnerProductLayer(name="ip5",output_dim=100,neuron=Neurons.LReLU(),bottoms=[:ip4],tops=[:ip5])
    ...
    exp_dir = "snapshots"
    solver_method = SGD()
    params = make_solver_parameters(solver_method, max_iter=20000, regu_coef=0.0005,
    mom_policy=MomPolicy.Fixed(0.9),
   # lr_policy=LRPolicy.Fixed(0.1))
    lr_policy=LRPolicy.Step(0.00001, 0.1,500))
    ...
    test_train_net = Net("TweetDetect-test", backend, [data_test, common_layers..., accuracy])
    add_coffee_break(solver, ValidationPerformance(test_train_net), every_n_iter=1000)
    
    data_test =  MemoryDataLayer(name="test-data",data=Array[x_test[:, 1:10000], y_test[1:10000]],batch_size=50)
    ...
    test_net = Net("TweetDetect-test", backend, [data_test, common_layers..., accuracy,netplot,rocplot,outputLayer])
    add_coffee_break(solver, ValidationPerformance(test_net), every_n_iter=2000)
...
function TestTest(net_output_in,y_test)
    ...
    for i in (repetit_size+1):size(hz,2)
        append!(predicted_truth,find(sum(hz[:,(i-repetit_size):i],2) .>=(repetit_size  )))
    end
