...
function  setupTrainingVectorBinary(x,y)
    #Find balance Sets of labels
    idySetPos = vcat(find(i-> i== 1,y))
    idySetNeg = rand(vcat(find(i-> i != 1,y)),4*length(idySetPos))
...
function MLLearn()
    ...
    fc1   = InnerProductLayer(name="ip1",output_dim=10000,neuron=Neurons.LReLU(),bottoms=[:data], tops=[:ip1], weight_init=GaussianInitializer(std=10))
    fc2   = InnerProductLayer(name="ip2",output_dim=1000,neuron=Neurons.LReLU(),bottoms=[:ip1], tops=[:ip2])
    fc3   = InnerProductLayer(name="ip3",output_dim=500,neuron=Neurons.LReLU(),bottoms=[:ip2],tops=[:ip3])
    fc4   = InnerProductLayer(name="ip4",output_dim=3,neuron=Neurons.LReLU(),bottoms=[:ip3],tops=[:final])
...
#    fc6   = InnerProductLayer(name="ip6",output_dim=3,neuron=Neurons.ReLU(),bottoms=[:ip5],tops=[:final])
    ...
    init(backend)
    common_layers = [fc1,fc2,fc3,fc4]
    net = Net("TweetDetect-train", backend, [data, common_layers..., loss])
    
    exp_dir = "snapshots"
    solver_method = SGD()
    params = make_solver_parameters(solver_method, max_iter=2000, regu_coef=0.0005,
    mom_policy=MomPolicy.Fixed(0.0),
   # lr_policy=LRPolicy.Fixed(0.1))
    lr_policy=LRPolicy.Inv(0.005, 0.001,0.75))
...
#    rocplot = ROCPlotLayer(name="train-net",bottoms=[:final, :label])
    ...
    add_coffee_break(solver, ValidationPerformance(test_train_net), every_n_iter=500)
    
    data_test =  MemoryDataLayer(name="test-data",data=Array[x_test[:,1:100000], y_test[1:100000]],batch_size=50)
...
#    outputLayer = MemoryOutputLayer(name="testPos-Output",bottoms=[:final])
    ...
    solve(solver, net) 
    net_output = 0#solver.coffee_lounge.coffee_breaks[3].coffee.validation_net.states[end].outputs
